...
class Opt:
    ...
    def initialize(self):
        ...
        self.parser.add_argument(
            ...
            type=str,
            default=".",
            help="root directory path of project",
        )
        self.parser.add_argument(
            "--dataset_path",
            type=str,
            default="datasets/coco/jpg",
            help="directory path of dataset",
        )
        self.parser.add_argument(
            "--annotation_path",
            type=str,
            default="datasets/coco/annotation",
            help="file path of annotations",
        )
        self.parser.add_argument(
            "--checkpoint_path",
            type=str,
        ...
        self.parser.add_argument(
            "--manual_seed", type=int, default=42, help="manual_seed of pytorch"
        )
        self.parser.add_argument(
        ...
        self.parser.add_argument(
            "--dataset",
            default="coco",
            help="specify the type of custom dataset to create",
        )
        self.parser.add_argument(
            "--checkpoint_interval",
            type=int,
        ...
        self.parser.add_argument(
            "--val_interval", type=int, default=5, help="evaluation every 5 epochs"
        )

        self.parser.add_argument(
            "--model", type=str, default="yolo_nano", help="choose which model to use"
        )
        self.parser.add_argument(
            "--image_size", type=int, default=416, help="size of image"
        )
        self.parser.add_argument(
            "--num_classes", type=int, default=80, help="# of classes of the dataset"
        )
        self.parser.add_argument(
            "--num_epochs", type=int, default=20, help="# of epochs"
        )
        self.parser.add_argument(
            "--begin_epoch", type=int, default=0, help="# of epochs"
        )
        ...
        self.parser.add_argument(
            ...
            type=str,
            default="Adam",
            help="optimizer (Adam | SGD | AdaBound)",
        )
        ...
        self.parser.add_argument(
            ...
            type=float,
            default=0.1,
            help="final learning rate used by AdaBound optimizer",
        )

        # object detection options
        self.parser.add_argument("--conf_thres", type=float, default=0.5)
        self.parser.add_argument("--nms_thres", type=float, default=0.5)

        self.parser.add_argument(
            "--no_multi_scale",
            action="store_true",
            help="if true, no multi-scale augmentation",
        )
        self.parser.set_defaults(no_multi_scale=False)
        self.parser.add_argument(
            "--no_pad2square",
            action="store_true",
            help="if true, no pad to square augmentation",
        )
        ...
        self.parser.add_argument(
            "--no_hflip",
            action="store_true",
            help="if true, no random horizontal-flip augmentation",
        )
        ...
        self.parser.add_argument(
            ...
            type=float,
            default=0.5,
            help="the probability of flipping the image and bboxes horozontally",
        )
        ...
        # visualizer
        self.parser.add_argument(
            "--no_vis", action="store_true", help="if true, no visualization"
        )
        self.parser.set_defaults(no_vis=False)
        self.parser.add_argument(
            "--no_vis_gt",
            action="store_true",
            help="if true, no visualization for ground truth",
        )
        self.parser.set_defaults(no_vis_gt=False)
        self.parser.add_argument(
            "--no_vis_preds",
            action="store_true",
            help="if true, no visualization for predictions",
        )
        self.parser.set_defaults(no_vis_preds=False)
        self.parser.add_argument(
            "--vis_all_images",
            action="store_true",
            help="if true, visualize all images in a batch",
        )
        ...
        self.parser.add_argument(
            "--print_options", default=True, help="print options or not"
        )
    ...
    def parse(self):
        ...
        if self.opt.project_root != "":
            self.opt.dataset_path = os.path.join(
                self.opt.project_root, self.opt.dataset_path
            )
            self.opt.checkpoint_path = os.path.join(
                self.opt.project_root, self.opt.checkpoint_path
            )
            if self.opt.resume_path:
                self.opt.resume_path = os.path.join(
                    self.opt.project_root, self.opt.resume_path
                )
        ...
        self.opt.device = (
            torch.device("cpu") if self.opt.no_cuda else torch.device("cuda")
        )
