    ...
    def plot_metrics(self, metrics, x, env="metrics"):
        for name, y in metrics:
            if name not in self.plots:
                self.plots[name] = self.viz.line(
                    X=np.array([x, x]),
                    Y=np.array([y, y]),
                    env=env,
                    opts=dict(title=name, xlabel="Batches", ylabel=name),
                )
            else:
                self.viz.line(
                    X=np.array([x]),
    ...
    def plot_ground_truth(self, images, bboxes, env="main"):
        ...
        for i in range(images.size(0)):
            ...
            if name not in self.plots:
                ...
                self.viz.matplot(
                    plt, win=self.plots[name], env=env, opts=dict(title=name)
                )
    ...
    def plot_predictions(self, images, detections, env="main"):
        ...
        for i in idx:
            ...
            if detection is None:
                text = (
                    "No bounding box found with confidence threshold - {} %.2f".format(
                        self.opt.conf_thres
                    )
                )
                if name not in self.plots:
                    self.plots[name] = self.viz.text(
                        text, env=env, opts=dict(title=name)
                    )
                else:
                    self.viz.text(
                        text, win=self.plots[name], env=env, opts=dict(title=name)
                    )
                continue
