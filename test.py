...
def test(model, dataloader, epoch, opt, test_logger, visualizer=None):
    ...
    for i, (images, targets) in enumerate(dataloader):
        ...
        sample_matrics += get_batch_statistics(
            detections, targets.cpu(), iou_threshold=0.5
        )

        if visualizer is not None and not opt.no_vis_preds:
            visualizer.plot_predictions(
                images.cpu(), detections, env="main"
            )  # plot prediction
    ...
    metric_table = AsciiTable(
        metric_table_data, title="[Epoch {:d}/{:d}".format(epoch, opt.num_epochs)
    )
