...
def _validate_bboxes(bboxes):
    if isinstance(bboxes, tuple) or isinstance(bboxes, np.ndarray):
        ...
        raise ValueError(
            "Type of bboxes should be `tuple`, `numpy.ndarray` or `torch.Tensor`. Got {}".format(
                type(bboxes)
            )
        )
    ...
    if bboxes.ndimension() != 2:
        raise ValueError(
            "Dimensions of bbox should be 2. Got {}".format(bboxes.ndimension())
        )
    # if bboxes.size(0) == 0:
    #     raise ValueError("There should be at least one bounding box. Got {}".format(bboxes.size(0)))
    if bboxes.size(-1) != 5:
        raise ValueError(
            "Last dimenion of bboxes should be 5 (including classes). Got {}".format(
                bboxes.size(-1)
            )
        )
...
class BBox(object):
    ...
    def to_tensor(self, mode="yolo"):
        if mode not in ("yolo", "xyhw", "xyxy", "coco"):
            raise ValueError(
                "BBox only supports mode: `yolo`, `xyhw`, `xyxy`, `coco`. Got {}".format(
                    mode
                )
            )
    ...
    def pad(self, padding):
        ...
        padded_bboxes = torch.cat(
            (classes, padded_xmin, padded_ymin, padded_xmax, padded_ymax), -1
        )
        w, h = self.size
    ...
    def rotate(self, angle):
        ...
        rotated_xmin = (
            torch.cos(angle) * (xmin - tx) - torch.sin(angle) * (ymin - ty) + tx
        )
        rotated_ymin = (
            torch.sin(angle) * (xmin - tx) + torch.cos(angle) * (ymin - ty) + ty
        )
        rotated_xmax = (
            torch.cos(angle) * (xmax - tx) - torch.sin(angle) * (ymax - ty) + tx
        )
        rotated_ymax = (
            torch.sin(angle) * (xmax - tx) + torch.cos(angle) * (ymax - ty) + ty
        )
    ...
    def transpose(self, method):
        if method not in (FLIP_LEFT_RIGHT, FLIP_TOP_BOTTOM):
            raise NotImplementedError(
                "Only horizontal and vertical flipping are supported. Got {}".format(
                    method
                )
            )
